  ...
  template: `
    <div class="container" *ngIf="item">
      ...
      <article style="max-width: 800px; margin: 0 auto;">
        ...
        <!-- Post Header -->
        <header style="margin-bottom: 48px; border-bottom: 1px solid var(--border-glass); padding-bottom: 32px;">
        ...
        <div *ngIf="item.image" style="margin-bottom: 48px; border-radius: var(--radius-md); overflow: hidden; box-shadow: var(--shadow-lg);">
...
export class BlogDetailComponent implements OnInit {
  ...
  // A robust, secure, lightweight Markdown-to-HTML parser function
  private parseMarkdown(md: string): string {
    if (!md) return '';
    let html = md;
    
    // 1. Sanitize to prevent malicious script injection (XSS protection)
    html = html
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");

    // Re-allow quote characters inside code blocks/tags
    // 2. Parse Code Blocks: ```lang \n content ```
    html = html.replace(/```(\w*)\n([\s\S]*?)```/g, (match, lang, code) => {
      // Decode inside code blocks so they print correctly
    ...
    // 4. Parse Headers (h1, h2, h3)
    html = html.replace(/^## (.*?)$/gm, '<h2>$1</h2>');
    html = html.replace(/^### (.*?)$/gm, '<h3>$1</h3>');
    html = html.replace(/^# (.*?)$/gm, '<h1>$1</h1>');

    // 5. Parse Bold Emphasis: **text**
    html = html.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');

    // 6. Parse Hyperlinks: [text](url)
    html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, text, url) => {
      const decodedUrl = url.replace(/&amp;/g, '&');
      return `<a href="${decodedUrl}" target="_blank" style="color: var(--primary); text-decoration: underline;">${text}</a>`;
    });

    // 7. Parse Bullet Points: - item
    // Temporarily replace list items
    html = html.replace(/^\- (.*?)$/gm, '<li>$1</li>');
    
    // Group adjacent <li> tags into <ul> tags
    html = html.replace(/((?:<li>.*?<\/li>\s*)+)/gs, '<ul>$1</ul>');

    // 8. Wrap standard paragraphs in <p> tags by splitting double newlines
    const parts = html.split(/\n\n+/);
    html = parts.map(p => {
      const trimmed = p.trim();
      if (!trimmed) return '';
      // Skip wrapping elements that are already block tags
      if (
