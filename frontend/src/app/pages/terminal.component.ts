    ...
    <div class="container" style="padding-top: 40px; min-height: 80vh;">
      <div style="text-align: center; margin-bottom: 32px;">
        ...
        <p style="max-width: 620px; margin: 0 auto;">
          Göktuğ Efe Madran'ın siber güvenlik laboratuvarına hoş geldin. Komutlar yardımıyla sistemi test edebilir, 
      ...
      <div class="terminal-window glass-panel" (click)="focusInput()">
        ...
        <div class="terminal-titlebar">
          ...
          <div class="terminal-title">gefe@security-lab:~</div>
...
export class TerminalComponent implements OnInit, AfterViewChecked {
  ...
  private showWelcomeMessage(): void {
    ...
    this.logs.push({ text: `╚═════╝ ╚══════╝╚═╝     ╚══════╝    ╚══════╝╚══════╝ ╚═════╝`, type: 'ascii' });
    this.logs.push({ text: `===============================================================`, type: 'ascii' });
    this.logs.push({ text: `GEFE SECURITY LABORATORY & TERMINAL OS v1.1.0`, type: 'success' });
    this.logs.push({ text: `Bağlantı Türü: Güvenli PostgreSQL (blog_db)`, type: 'info' });
    this.logs.push({ text: `Sistem Sahibi: Göktuğ Efe Madran (Siber Güvenlik Teknolojileri)`, type: 'info' });
  ...
  public onCommandSubmit(value: string): void {
    ...
    this.logs.push({ text: `gefe@sec-lab:~$ ${cleanValue}`, type: 'input' });
    this.history.push(cleanValue);
    this.historyIndex = -1;

    // Check if we are in Quiz Game Mode
    if (this.inQuizMode) {
  ...
  private executeCommand(cmd: string): void {
    ...
    switch (mainCmd) {
      ...
      case 'nmap':
        ...
        break;
      case 'ping':
        this.cmdPing(parts[1] || 'gefe35.dev');
        break;
      case 'date':
      ...
      case 'ls':
        ...
        break;
      case 'pwd':
        this.logs.push({ text: `/home/gefe35/portfolio`, type: 'info' });
        break;
      case 'echo':
        this.logs.push({ text: parts.slice(1).join(' '), type: 'info' });
        break;
      case 'sudo':
        this.logs.push({ text: `[sudo] password for gefe35: `, type: 'warning' });
        setTimeout(() => {
  ...
  private cmdHelp(): void {
    this.logs.push({ text: `=== GEFE SECURITY LAB — KOMUT LİSTESİ ===`, type: 'success' });
  ...
  private cmdWhoami(): void {
    this.logs.push({ text: `Göktuğ Efe Madran`, type: 'success' });
    this.logs.push({ text: `─────────────────────────────────────────────`, type: 'ascii' });
    ...
    this.logs.push({ text: `  E-posta : gefemadran@icloud.com`, type: 'info' });
    this.logs.push({ text: `  GitHub  : github.com/gefe35`, type: 'info' });
  }

  private cmdContact(): void {
    this.logs.push({ text: `--- İLETİŞİM BİLGİLERİ ---`, type: 'success' });
    this.logs.push({ text: `  E-posta : gefemadran@icloud.com`, type: 'info' });
    this.logs.push({ text: `  GitHub  : https://github.com/gefe35`, type: 'info' });
    this.logs.push({ text: `  Konum   : İzmir, Türkiye`, type: 'info' });
  ...
  private cmdAbout(): void {
    ...
    this.logs.push({ text: `İzmir'de yaşayan, 20 yaşında meraklı bir Siber Güvenlik öğrencisiyim. Konak Kavram Meslek Yüksekokulu siber güvenlik laboratuvarlarında ağ güvenliği, sızma testleri ve web zafiyetleri üzerine yoğunlaşıyorum.`, type: 'info' });
  ...
  ( o.o )    -----------------
   > ^ <     OS: İzmir Cyber Security OS v1.0.4
             ...
             Shell: zsh (Gefe-Security-Shell)
             Uptime: 20 years, 2 months
             Packages: npm (12), pip (8), nuget (4)
             CPU: Göktuğ Efe Madran (1 Core / 4 Threads)
  ...
  private cmdProjects(): void {
    ...
    this.githubService.getRepositories().subscribe({
      next: (repos) => {
        this.logs.push({ text: `Başarılı! Göktuğ Efe Madran'ın Canlı GitHub Repoları:`, type: 'success' });
        this.logs.push({ text: `----------------------------------------------------------------------`, type: 'ascii' });
  ...
  private startQuiz(): void {
    ...
    this.logs.push({ text: `Göktuğ'un siber dünyasından 5 seçkin soru seni bekliyor.`, type: 'info' });
  ...
  private finishQuiz(): void {
    ...
    if (this.quizScore === this.quizQuestions.length) {
      this.logs.push({ text: `🏆 MÜKEMMEL! Tam puan aldın! Göktuğ Efe Madran onaylı Siber Güvenlik Rozetini kazandın!`, type: 'success' });
      this.logs.push({ text: `Rozet Kodu: [GEFE-SEC-CHAMPION-2026]`, type: 'success' });
      ...
      this.logs.push({ text: `Kendini geliştirmek için Göktuğ'un blog yazılarını okuyabilirsin!`, type: 'info' });
    }
